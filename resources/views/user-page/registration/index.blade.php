...
@push('scripts')
@vite(['resources/js/registration/referral.js', 'resources/js/registration/form-handler.js', 'resources/js/registration/load-studyprogram.js'])
<script type="module">
    import {
        initReferralHandler
    } from "{{ Vite::asset('resources/js/registration/referral.js') }}";
    import {
        initFormHandler
    } from "{{ Vite::asset('resources/js/registration/form-handler.js') }}";
    import {
        loadStudyPrograms
    } from "{{ Vite::asset('resources/js/registration/load-studyprogram.js') }}";
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const DOM = {
            ...
            generalErrorContainer: document.getElementById('form-general-error'),
            studyProgramSelect: studyProgramSelectElement
        };

        const updatePaymentUI = (discountAmount) => {
            const validDiscount = Math.min(discountAmount, baseFee);
            const finalAmount = baseFee - validDiscount;

            if (validDiscount > 0 && DOM.originalAmountText) {
                DOM.originalAmountText.classList.remove('hidden');
                DOM.originalAmountText.innerText = `IDR ${baseFee.toLocaleString('id-ID')}`;
            } else if (DOM.originalAmountText) {
                DOM.originalAmountText.classList.add('hidden');
            }

            if (DOM.finalAmountText) {
                DOM.finalAmountText.innerText = `IDR ${finalAmount.toLocaleString('id-ID')}`;
            }

            if (DOM.paymentProofInput) {
                DOM.paymentProofInput.required = (finalAmount > 0);
            }
        };
